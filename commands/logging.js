...
exports.run = (bot, message, args) => {
    if(message.author.id == config.botOwner) {
        if(args.length >= 1) {
            if (args.length <= 1) {
                let newLoggingChannel = message.content.split(" ").slice(1, 2)[0];
                config.loggingChannel = newLoggingChannel;
                ...
                message.channel.send ({embed: {
                    "color": 16762967,
                    "title": "Cambiaste el canal de logging a `" + newLoggingChannel + "`",
                }});
            } else {
                ...
                message.channel.send ({embed: {
                    "color": 15806281,
                    "title": message.author.username + ", tan solo debes proporcionar un ID de canal",
                }});
            }
        } else {
            ...
            message.channel.send ({embed: {
                "color": 15806281,
                "title": message.author.username + ", debes proporcionar un ID de canal",
            }});
        }
    } else {
