exports.run = async (discord, fs, client, message, args, command) => {
    ...
    try {
        ...
        let helpEmbed = new discord.MessageEmbed()
            ...
            .addField(`:stop_button: ${client.config.prefixes.staffPrefix}stop`, `Detiene a ${client.user.username} ${client.emotes.shield}`)
            ...
            .addField(`🔇 ${client.config.prefixes.staffPrefix}mute (@usuario | id) (motivo)`, `Silencia a un miembro${client.emotes.shield}`)
            ...
            .addField(`🔊 ${client.config.prefixes.staffPrefix}unmute (@usuario | id) (motivo)`, `Des-silencia a un miembro${client.emotes.shield}`)
            .addField(`⛔ ${client.config.prefixes.staffPrefix}kick (@usuario | id)) (motivo)`, `Expulsa a un miembro${client.emotes.shield}`)
            .addField(`:hammer: ${client.config.prefixes.staffPrefix}ban (@usuario | id) (motivo)`, `Banea a un usuario${client.emotes.shield}`)
            .addField(`🕑 ${client.config.prefixes.staffPrefix}tempban (@usuario | id) (xS | xM | xH | xD) (motivo)`, `Banea a un usuario de forma temporal${client.emotes.shield}`)
            .addField(`🛡 ${client.config.prefixes.staffPrefix}softban (@usuario | id) (1 - 7) (motivo)`, `Banea a un usuario y borra sus mensajes (máx 7 días)${client.emotes.shield}`)
            .addField(`${client.emotes.greenTick} ${client.config.prefixes.staffPrefix}unban (id) (motivo)`, `Desbanea a un usuario${client.emotes.shield}`)
            ...
            .addField(`🏆 ${client.config.prefixes.staffPrefix}xp (@miembro | id) (set | add | remove | clear) [cantidad]`, `Modifica la cantidad de XP de un miembro${client.emotes.shield}`)
