exports.run = async (discord, fs, client, message, args, command, supervisorsRole, noPrivilegesEmbed) => {
    ...
    try {
        let notToMuteEmbed = new discord.MessageEmbed()
            ...
            .setDescription(`${client.emotes.redTick} No se ha encontrado al miembro. Debes mencionarlo o escribir su id`);
        ...
        if (message.author.id !== client.config.guild.botOwner) {
            let maxTimeEmbed = new discord.MessageEmbed()
                .setColor(client.colors.red2)
                .setDescription(`${client.emotes.redTick} Los moderadores solo pueden silenciar un máximo de 1 día`);
        ...
        if (message.author.id !== client.config.guild.botOwner) {
            let undefinedReasoneEmbed = new discord.MessageEmbed()
                .setColor(client.colors.red2)
                .setDescription(`${client.emotes.redTick} Los moderadores deben adjuntar una razón`);
