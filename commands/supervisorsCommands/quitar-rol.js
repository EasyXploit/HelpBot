    ...
    if(message.author.id === config.botOwner) {
        if(args.length >= 2) {
            if (args.length <= 2) {
                        ...
                        async function addRole() {
                            try {
                                ...
                                let successEmbed = new discord.RichEmbed()
                                    ...
                                    .setDescription('Retiraste el rol ' +  args[0] + ' al usuario **' + args[1] + '**.');
                                ...
                                console.log('\n 》' + message.author.username + ' retiró el rol ' + message.mentions.roles.first().name + ' al usuario ' + args[1] + '.');
                ...
                let errorEmbed = new discord.RichEmbed()
                    .setColor(0xF12F49)
                    .setDescription('❌ Proporcionaste demasiados argumentos.\nTan solo debes escribir el rol y el usuario que lo perderá');
                message.channel.send(errorEmbed);
            }
        } else {
            ...
            let errorEmbed = new discord.RichEmbed()
                .setColor(0xF12F49)
                .setDescription('❌ Debes escribir el rol y el usuario que lo perderá');
            message.channel.send(errorEmbed);
        }
                        ...
                        async function addRole() {
                            try {
                                ...
                                if (roleID === roles.iniciados || roleID === roles.profesionales || roleID === roles.veteranos || roleID === roles.expertos) {
                                    ...
                                    console.log (new Date().toUTCString() + ' 》' + message.author.username + ' no dispone de privilegios suficientes para retirar el rol ' + message.mentions.roles.first() + ' en ' + message.guild.name);

                                    let errorEmbed = new discord.RichEmbed()
                                        .setColor(0xF12F49)
                                        .setTitle('❌ Ocurrió un error')
                                        .setDescription('❌ ' + message.author.username + ', no dispones de privilegios suficientes para retirar este rol');
                                    message.channel.send(errorEmbed);
                                    return;
                                }
                                ...
                                let successEmbed = new discord.RichEmbed()
                                    .setColor(0xB8E986)
                                    .setTitle('✅ Operación completada')
                                    .setDescription('Eliminaste el rol ' +  args[0] + ' al usuario **' + args[1] + '**.');
                                message.channel.send(successEmbed);
