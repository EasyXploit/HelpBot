exports.run = async (client, message, args, command, commandConfig, locale) => {
    ...
    try {
        ...
        await message.channel.send({ embeds: [ new client.MessageEmbed()
            ...
            .addField(`🌝 ${locale.embed.stickersAndEmojis}`, `${client.functions.localeParser(locale.embed.totalesEmojis, { totalesEmojis: (await message.guild.emojis.fetch()).size.toString() })}\n${client.functions.localeParser(locale.embed.totalStickers, { totalStickers: (await message.guild.stickers.fetch()).size.toString() })}`, true)
