export async function run(interaction, commandConfig, locale) {
    ...
    try {
        ...
        if (interaction.options._hoistedOptions[1] && !channel) return interaction.reply({ embeds: [ new discord.MessageEmbed()
        ...
        if (!msg) return interaction.reply({ embeds: [ new discord.MessageEmbed()
            .setColor(`${await client.functions.db.getConfig('colors.error')}`)
        ...
        if (msg.author.id !== client.user.id) return interaction.reply({ embeds: [ new discord.MessageEmbed()
            .setColor(`${await client.functions.db.getConfig('colors.error')}`)
