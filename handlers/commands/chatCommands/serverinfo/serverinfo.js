export async function run(interaction, commandConfig, locale) {
    ...
    try {
        ...
        let resultEmbed = new discord.EmbedBuilder()
            ...
            .addFields({ name: `💬 ${locale.embed.channels}`, value: `${await client.functions.utils.parseLocale(locale.embed.totalCategories, { totalCategories: categories.size - 1 })}\n${await client.functions.utils.parseLocale(locale.embed.totalTextChannels, { totalTextChannels: guildChannels.filter(c => c.type === 'GuildText').size })}\n${await client.functions.utils.parseLocale(locale.embed.totalVoiceChannels, { totalVoiceChannels: guildChannels.filter(c => c.type === 'GuildVoice').size })}`, inline: true })
