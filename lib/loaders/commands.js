...
export async function loadCommands() {
    ...
    for (const commandType of await fs.readdirSync('./handlers/commands/')) {
        ...
        for (const command of remoteCommands) {
            ...
            //Comprueba si el tipo de comando coincide con la configuración local, y no es un comando a ignorar
        ...
        for (const commandName of await fs.readdirSync(`./handlers/commands/${commandType}/`)) {
            ...
            //Crea un objeto para almacenar los datos del comando
            let newCommandData = {
                config: lodash.cloneDeep(importedCommandData.config),
                run: importedCommandData.run,  //'run' es una función, por lo que no se clona
            };
            ...
            if (commandType === 'chatCommands') {
                ...
                if (localCmd.appData.options) for (const option of localCmd.appData.options) {
                    ...
                    //Normaliza los campos de nombre y descripción
            ...
            //Almacena la configuración de usuario del comando en los datos del comando
