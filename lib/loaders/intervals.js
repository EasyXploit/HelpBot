...
export async function loadIntervals() {
    ...
    setInterval(async () => {
        ...
        for (let timeoutData of memberTimeouts) {
            ...
            if (member) await member.send({ embeds: [ new discord.MessageEmbed()
                .setColor(`${await client.functions.db.getConfig('colors.correct')}`)
    ...
    setInterval(async () => {
        ...
        for (let pollData of currentPolls) {
            ...
            if (Date.now() > pollData.expirationTimestamp) {
                ...
                //Comprueba si el bot tiene los permisos requeridos para enviar los resultados, y los envía si los tiene
                const missingPermissions = await client.functions.utils.missingPermissions(null, client.baseGuild.members.me, ['SEND_MESSAGES', 'EMBED_LINKS', 'ATTACH_FILES']);
                ...
                else await poll.channel.send({ embeds: [ new discord.MessageEmbed()
                ...
                await client.functions.managers.sendLog('pollEnded', 'embed', new discord.MessageEmbed()
                ...
                if (oldRemainingTime !== newRemainingTime) await poll.edit({ embeds: [ new discord.MessageEmbed()
