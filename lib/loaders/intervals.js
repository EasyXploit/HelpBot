...
export async function loadIntervals() {
    ...
    setInterval(async () => { await checkTimeouts(); }, 5000);

    // Sets the interval to check the bans
    setInterval(async () => { await checkBans(); }, 5000);

    // Sets the interval to check the websocket response time
    setInterval(async () => { await checkPing(); }, 60000);

    // Sets the interval to check the polls
    setInterval(async () => { await checkPolls(); }, 5000);

    // Sets the interval to update the voice time
    setInterval(async () => { await updateVoiceTime(); }, await client.functions.db.getConfig('leveling.XPGainInterval'));

    // Sets the interval to update the presence
    setInterval(async () => { await updatePresence(); }, 60000);

    // If the bot should check the usernames, sets the interval to do it
    if (await client.functions.db.getConfig('moderation.kickOnBadUsername')) setInterval(async () => { await checkUsernames(); }, 120000);
