...
export async function checkPolls() {
...
// Stores the interval IDs in an object
const intervals = {};
...
export async function loadIntervals() {
    ...
    intervals.checkTimeouts = {
        id: setInterval(async () => { await checkTimeouts(); }, 5000),
    ...
    intervals.checkBans = {
        id: setInterval(async () => { await checkBans(); }, 5000),
        function: checkBans,
        interval: 5000
    };
    ...
    intervals.updateVoiceTime = {
        id: setInterval(async () => { await updateVoiceTime(); }, await client.functions.db.getConfig('leveling.XPGainInterval')),
    ...
    if (await client.functions.db.getConfig('moderation.kickOnBadUsername')) intervals.checkUsernames = {
        id: setInterval(async () => { await checkUsernames(); }, 120000),
...
export function startInterval(name) {
    ...
    if (intervals[name] && !intervals[name].id) {

        // Starts the interval
        intervals[name].id = setInterval(intervals[name].function, intervals[name].interval);
    };
};
