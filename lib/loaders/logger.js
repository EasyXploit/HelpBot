...
export async function loadLogger() {
    ...
    //Genera un formato para los logs de advertencia
    const warnFormat = winston.format.printf(({ level, message, label, timestamp }) => {
        return `${timestamp} [${label}] ${level}: ${message}`;
    });
                ...
                new winston.transports.Console({
                    ...
                    format: winston.format.combine(
                        ...
                        winston.format.printf(({ level, message, label, timestamp }) => {
                            switch (level) {
                                case 'debug':
                                    return debugFormat({ level, message, label, timestamp });
                                case 'warn':
                                    return warnFormat({ level, message, label, timestamp });
                                case 'error':
                                    return errorFormat({ level, message, label, timestamp });
                                default:
                                    return `${timestamp} [${label}] ${level}: ${message}`;
                            }
                        })
