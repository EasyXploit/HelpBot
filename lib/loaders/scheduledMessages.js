...
export async function loadScheduledMessages() {
    ...
    async function sendMessage(messageConfig, channel) {
        ...
        if (lastSentMsg) {
            ...
            // Stores an array with the Id's of the messages
    ...
    for (let configuredMessage of configuredMessages) {
        ...
        // Generates an array to store the intervals
        client.scheduledMessageIntervals = [];

        // Programs the message to be sent periodically
        const interval = setInterval(async () => { sendMessage(configuredMessage, channel) }, configuredMessage.interval);

        // Stores the interval in the client
        client.scheduledMessageIntervals.push(interval);
    };

    // Stores the scheduled messages sent
    const sentScheduledMessaged = await client.functions.db.getData('sent');

    // For each of the sent messages
    for (const sent of sentScheduledMessaged) {
    ...
    logger.debug('Scheduled messages loading completed');
};
