...
export async function loadTimedMessages() {
    ...
    async function sendMessage(messageConfig, channel) {
        ...
        const missingPermissions = await client.functions.utils.missingPermissions(channel, channel.guild.members.me, ['SEND_MESSAGES', 'EMBED_LINKS', 'ATTACH_FILES', 'READ_MESSAGE_HISTORY'], true);
    ...
    for (let configuredMessage of configuredMessages) {
        ...
        if (!configuredMessage.hash) {
            ...
            //Genera un hash MD5 a partir de la config. del mensaje programado
