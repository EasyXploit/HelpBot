...
export default async (member, reason, action, moderator, message, interaction, channel, filteredURL) => {
    ...
    try {
        ...
        if (channel.type !== 'DM') {
            ...
            if (interaction) {
                ...
                await interaction.reply({ embeds: [ new client.MessageEmbed()
                    .setColor(`${await client.functions.db.getConfig('colors.warning')}`)
                    .setDescription(`${client.customEmojis.orangeTick} ${await client.functions.utils.parseLocale(locale.warn.notificationEmbed, { memberTag: member.user.tag, warnReason: warnReason })}.`)
                ]});
        ...
        if (action === 2 || action === 3) {
            ...
            const warnId = await client.functions.utils.generateSid.default();
