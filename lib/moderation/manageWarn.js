...
export default async (member, reason, action, moderator, message, interaction, channel, filteredURL) => {
    ...
    try {
        ...
        async function timeout(duration) {
            ...
            const missingPermission = await client.functions.utils.missingPermissions(null, client.baseGuild.me, ['MODERATE_MEMBERS']);
            ...
            if (channel.type !== 'DM') await channel.send({ embeds: [ new discord.MessageEmbed()
        ...
        async function kick() {
            ...
            const missingPermission = await client.functions.utils.missingPermissions(null, client.baseGuild.me, ['KICK_MEMBERS']);
        ...
        async function ban(duration) {
            ...
            const missingPermission = await client.functions.utils.missingPermissions(null, client.baseGuild.me, ['BAN_MEMBERS']);
        ...
        if (channel.type !== 'DM') {
            ...
            const missingExtEmojisPermissions = interaction || channel.type !== 'DM' ? await client.functions.utils.missingPermissions(channel, client.baseGuild.me, ['USE_EXTERNAL_EMOJIS']) : false;
            ...
            if (interaction) {
                ...
                if (channel.type !== 'DM') await message.channel.send({ embeds: [ new discord.MessageEmbed()
                    .setColor(`${await client.functions.db.getConfig('colors.warning')}`)
                    .setDescription(`${missingExtEmojisPermissions ? '' : client.customEmojis.orangeTick} ${await client.functions.utils.parseLocale(locale.warn.notificationEmbed, { memberTag: member.user.tag, warnReason: warnReason })}.`)
                ]});
            };
        ...
        else if (message.channel.type !== 'DM') logger.warn(`The bot does not have permission to delete the message with ID ${message.id} from ${message.author.tag} (${message.author.id}) on ${message.channel.name} (${message.channel.id})`);
        ...
        if (action === 2 || action === 3) {
            ...
            await client.functions.managers.sendLog('warnedMember', 'embed', new discord.MessageEmbed()
                .setColor(`${await client.functions.db.getConfig('colors.warning')}`)
                .setAuthor({ name: await client.functions.utils.parseLocale(locale.warn.loggingEmbed.author, { memberTag: member.user.tag }), iconURL: member.user.displayAvatarURL({dynamic: true}) })
                .addFields(
            ...
            if (message && await client.functions.db.getConfig('moderation.attachFilteredMessages')) await client.functions.managers.sendLog('warnedMember', 'file', new discord.MessageAttachment(Buffer.from(filteredURL || message.content, 'utf-8'), `filtered-${Date.now()}.txt`));
